...
for lr in xrange(len(net.layer)):
  ...
  net.layer[lr].v = np.zeros(net.layer[lr].N)
  net.layer[lr].u = np.zeros(net.layer[lr].N)
  net.layer[lr].s = np.zeros((4,net.layer[lr].N))
  net.layer[lr].s[0,:] = -65.0*np.ones(net.layer[lr].N)
...
for t in xrange(T):
   ...
   # net.layer[0].I = Ib * np.random.rand(N1)
   net.layer[0].I = Ib * np.ones(N1)
   net.layer[1].I = net.layer[0].v*0.2
