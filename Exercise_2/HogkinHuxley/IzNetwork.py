...
class IzNetwork:
  ...
  def NeuronUpdate(self, i, t):
    ...
    for k in xrange(int(1/dt)):
      ...
      self.layer[i].s += dt*HH.s_dt(s,self.layer[i].I,self.layer[i].params)
      self.layer[i].v = self.layer[i].s[0,:]
      self.layer[i].u = self.layer[i].s[1,:]

      # Find index of neurons that have fired this millisecond
      fired = np.where(self.layer[i].v >= 60)[0]
